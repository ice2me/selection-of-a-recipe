...
function InputEnter() {
	...
	const submitHandler = async (e) => {
		...
		try {
			const fetched = await request(
				'https://selection-recipe.herokuapp.com/api/search',
				'POST',
				inpIngredient
			);
	...
	useEffect(() => {
		try {
			request('https://selection-recipe.herokuapp.com/api/tags/').then(
				(res) => {
					setOptionList(res);
				}
			);
		...
		<div className='form'>
			<InputGroup>
				...
				<Button
					...
					disabled={
						inpIngredient.length === 0 || inpIngredient[0] === ''
					}>
					<svg
						width='19'
						height='18'
			...
			{addRecipeModal && (
				<AddRecipe closeAddRecipeModal={closeAddRecipeModal} />
			)}
			{dishPartly.length > 0 ||
			dishFull.length > 0 ||
			inpIngredient.length >= 1 ? (
				dishPartly.length < 1 &&
				dishFull.length < 1 &&
				inpIngredient.length > 0 ? (
