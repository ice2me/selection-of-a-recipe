...
function AddRecipe({ closeAddRecipeModal }) {
	...
	const addNewIngredientsHandler = () => {
		setIngredients(
			ingredients.concat([{ testId: Date.now(), name: '', quantity: '' }])
		);
	};
	const deleteHandler = (id) => {
		setIngredients(
			ingredients.filter((ingredient) => ingredient.testId !== id)
		);
	};
	useEffect(() => {
		setIngredients(
	...
	const submitHandler = async (e) => {
		...
		const payload = {
			...
			ingredients: ingredients.filter(
				(ingr) => ingr.name !== '' && delete ingr.testId
			),
			steps: descriptions.filter(
				(des) => des.recipeDescription !== '' && delete des.testId
			)
		};
		try {
			await request(
				'http://localhost:5000/api/recipe/dish',
				'POST',
				payload
			);
		} catch (e) {
	...
	const addNewTextarea = () => {
		setDescriptions(
			descriptions.concat([{ testId: Date.now(), recipeDescription: '' }])
		);
	};
		...
		<div className='add-recipe'>
			...
			{loading ? (
				...
				<form autoComplete='new-password'>
					...
					<label htmlFor='steps' className='mt-3'>
						...
						{descriptions.map((des, index) => (
							<AddBlokcTextarea
								...
								isAddVisibleTextarea={
									descriptions.length - 1 === index
								}
								isDeleteTextarea={descriptions.length > 1}
							/>
						))}
					</label>
					<p className='pb-2'>* обязательное поле для заполнения</p>
					{recipeName &&
					recipePhoto &&
					ingredients &&
					descriptions ? (
						<Button
							style={{
								backgroundColor: 'rgba(237,174,1, 1)',
								border: 'none'
							}}
							type='button'
						...
						<Button
							style={{
								backgroundColor: 'rgba(142,104,89, 1)',
								border: 'none'
							}}
							type='button'
							disabled={true}
